...
pub const GGuttman = galaxy.Galaxy(PageCache, .guttman);
pub const GLinear = galaxy.Galaxy(PageCache, .linear);
pub const GHybrid = galaxy.Galaxy(PageCache, .hybrid);
pub const G = GHybrid; // the default strategy the existing tests exercise
...
pub const Collector = struct {
    ...
    pub fn init(alloc: std.mem.Allocator) !Collector {
        return .{
            .alloc = alloc,
            .items = try std.ArrayList(Rec).initCapacity(alloc, 0),
        };
    }
    ...
    pub fn cb(self: *Collector, mbr: anytype, value: []const u8) anyerror!void {
        ...
        try self.items.append(
            self.alloc,
            .{ .id = s.id.get(), .x = mbr.low[0], .y = mbr.low[1] },
        );
    }
