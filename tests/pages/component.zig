...
comptime {
    pages.assertTrait(ValidTrait);
    pages.assertBinding(ValidTrait.Binding(TestBackend), TestBackend);
}

test "Pages: component descriptor preserves its exact trait" {
    const descriptor = pages.Descriptor{ .Trait = ValidTrait };
    try std.testing.expect(descriptor.Trait == ValidTrait);
}

test "Pages: page-kind range performs checked lookup" {
    const range = pages.PageKindRange{ .base = 0x0100, .count = 2 };

    try std.testing.expectEqual(@as(?pages.PageKind, 0x0100), range.kindAt(0));
    try std.testing.expectEqual(@as(?pages.PageKind, 0x0101), range.kindAt(1));
    try std.testing.expectEqual(@as(?pages.PageKind, null), range.kindAt(2));
    try std.testing.expectEqual(@as(u32, 0x0102), range.endExclusive());
}

test "Pages: page-kind range rejects integer overflow" {
    const range = pages.PageKindRange{
        .base = std.math.maxInt(pages.PageKind),
        .count = 2,
    };

    try std.testing.expectEqual(@as(?pages.PageKind, std.math.maxInt(pages.PageKind)), range.kindAt(0));
    try std.testing.expectEqual(@as(?pages.PageKind, null), range.kindAt(1));
    try std.testing.expectEqual(@as(?pages.PageKind, null), range.kindAt(std.math.maxInt(usize)));
}

test "Pages: binding exposes a borrowed typed proxy" {
