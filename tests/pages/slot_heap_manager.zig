...
const Manager = fullaz.pages.SlotHeapManager(
    Backend,
    Location,
    32,
    6,
...
const Descriptor = fullaz.pages.slotHeap(.{
    .compare = compare,
    .CompareContext = void,
    .comparator_id = 1,
    .maximum_key_size = 4,
...
test "Pages: SlotHeap descriptor composes its manager, FSM, and paged model" {
    const Binding = Descriptor.Trait.Binding(Backend);
    comptime fullaz.pages.assertBinding(Binding, Backend);
    comptime fullaz.pages.assertStaticMetadata(Binding, Binding.StaticMetadata);
...
test "Pages: SlotHeap memory database commits and rolls back" {
