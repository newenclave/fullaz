...
test "Bloom:: no false negatives for added keys" {
...
test "Bloom:: false-positive rate stays near target" {
    const alloc = std.testing.allocator;
    const n = 1000;
    var f = try Filter.init(alloc, n, 0.01);
    defer f.deinit(alloc);
...
test "Bloom:: no false negatives for added keys: BloomImpl" {
    const alloc = std.testing.allocator;
    const n = 1000;
    var f = try Filter.init(alloc, n, 0.01);
    defer f.deinit(alloc);
...
test "Bloom:: false-positive rate stays near target; BloomImpl" {
    const alloc = std.testing.allocator;
    const n = 1000;
    var f = try Filter.init(alloc, n, 0.01);
    defer f.deinit(alloc);
...
test "Bloom:: false-positive rate stays near target; BloomImpl with context" {
    const alloc = std.testing.allocator;
    const n = 1000;
    var f = try Filter.init(alloc, n, 0.01);
    defer f.deinit(alloc);
