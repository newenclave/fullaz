...
const weighted_bpt = @import("../../weighted_bpt/weighted_bpt.zig");
const weighted_seq = @import("../../storage/weighted_seq/weighted_seq.zig");
...
pub fn weightedSequence(comptime options: anytype) component.Descriptor {
    ...
    if (info != .@"struct" or info.@"struct".is_tuple) {
        @compileError("pages.weightedSequence options must be a named struct");
    }
    inline for (info.@"struct".fields) |field| {
        if (!std.mem.eql(u8, field.name, "maximum_chunk_size")) {
            @compileError("Unknown pages.weightedSequence option: " ++ field.name);
        }
    }
    ...
    switch (@typeInfo(ChunkSizeT)) {
        .int, .comptime_int => {},
        else => @compileError("pages.weightedSequence maximum_chunk_size must be an unsigned integer"),
    }
    const chunk_size = std.math.cast(usize, maximum_chunk_size) orelse
        @compileError("pages.weightedSequence maximum_chunk_size must fit usize");
    if (chunk_size == 0) {
        @compileError("pages.weightedSequence maximum_chunk_size must be non-zero");
    }
