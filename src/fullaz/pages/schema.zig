...
fn validateName(comptime name: []const u8) void {
    if (name.len == 0) {
        @compileError("Pages Schema component name cannot be empty");
    }
    if (name.len > 255) {
        @compileError("Pages Schema component name is too long");
    }
    if (!std.unicode.utf8ValidateSlice(name)) {
        @compileError("Pages Schema component name is not valid UTF-8");
    }
    if (std.mem.indexOfScalar(u8, name, 0) != null) {
        @compileError("Pages Schema component name cannot contain NUL");
    }
    for (name, 0..) |byte, index| {
        if (byte == '$' and (index == 0 or name[index - 1] == '/')) {
            @compileError("Pages Schema component name uses a reserved $ path segment");
        }
    }
}
...
pub fn Schema(comptime options: anytype) type {
    ...
    if (options_info != .@"struct" or options_info.@"struct".is_tuple) {
        @compileError("Pages Schema options must be a named struct");
    }
    if (!@hasField(@TypeOf(options), "page_id") or @TypeOf(options.page_id) != type) {
        @compileError("Pages Schema options must declare page_id as a type");
    }
    const page_id_info = @typeInfo(options.page_id);
    if (page_id_info != .int or page_id_info.int.signedness != .unsigned) {
        @compileError("Pages Schema page_id must be an unsigned integer");
    }
