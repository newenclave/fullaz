...
pub fn StaticDatabaseWithWal(comptime SchemaT: type, comptime DeviceT: type, comptime LogDeviceT: type) type {
    ...
    return struct {
        ...
        const GcSession = struct {
            ...
            pub const SessionError = Self.Error || GcCollector.Error;

            fn corePtr(self: *const SessionSelf) *Core {
                return self.raw.corePtr();
            }

            fn ensureCollector(self: *SessionSelf) SessionError!*GcCollector {
                if (!self.initialized) {
            ...
            pub fn collector(self: *SessionSelf) SessionError!*GcCollector {
                return self.ensureCollector();
            }

            pub fn start(self: *SessionSelf, roots: []const SchemaT.PageId) SessionError!void {
                const core = self.corePtr();
                if (core.gc_cycle_active) {
            ...
            pub fn step(self: *SessionSelf, maximum_pages: usize) SessionError!gc.StepStatus {
                const core = self.corePtr();
                if (!core.gc_cycle_active) {
            ...
            pub fn phase(self: *SessionSelf) SessionError!gc.Phase {
                _ = try self.collector();
                return self.model.phase();
            }

            pub fn abort(self: *SessionSelf) SessionError!void {
                const core = self.corePtr();
                if (!core.gc_cycle_active) {
                    return error.BadGcState;
                }
            ...
            pub fn deinit(self: *SessionSelf) void {
                if (self.active) {
                    self.rollback() catch @panic("StaticDatabase GC session rollback failed");
                }
                self.* = undefined;
            }
        };
